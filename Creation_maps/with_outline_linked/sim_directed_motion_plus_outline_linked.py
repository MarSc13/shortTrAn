...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  2 10:22:21 2020
...
def directed_motion_plus_outline(path,filename,outline_path,resultpath,binning, px_extent, N, highlighting):
    ...
    if not os.path.exists(resultpath+'/DirectedMotionMaps'):
            ...
            resultpath_save = resultpath+'/DirectedMotionMaps/'
    else:
            resultpath_save = resultpath+'/DirectedMotionMaps/'
    ...
    for i in range(1,N+1):
        ...
        '''Linking of the diffusion map with the speed heat map and the quiver plot.
        If the ellipticity exceeds the value 2 or falls below the value 0.5 
        then the entries in the heat map or quiver plot are preserved. Free 
        diffusion and directional motion cannot be present at the same time at 
        the same location, assuming the same conditions in the membrane at all times.'''
        
        ellip = np.load(resultpath + '/DiffusionMaps/Info/Ellipticity/ellipticity_cell_' + str(i) + '.npy')
        ellip_mask = np.zeros(ellip.shape)
        ellip_mask[ellip != 0] = 1 
        ellip_bin = np.zeros(ellip.shape)
        ellip_bin[ellip > 1.5] = 1
        ellip_bin[ellip < 0.75] = 1
        ellip_bin = np.multiply(ellip_bin, ellip_mask)
        img_x = np.multiply(ellip_bin,img_x)
        img_y = np.multiply(ellip_bin, img_y)
        img_l = np.multiply(ellip_bin,img_l)
        angle = np.multiply(ellip_bin, angle)
        ...
        if size_outline == 2:
            ...
            if highlighting == 'Yes':
                ...
                '''Generates quiver plot and heat map fpr the whole cell'''
                HeatMap_QuiverPlot_WholeCell(img_x, img_y, img_l, angle, outline,\
                                             outline, wbmap, resultpath_save, i)
    ...
    np.save(resultpath_save+'Speed_info_all_bin'+str(binning),speed_info)
    np.save(resultpath_save+'Speed_info_all_sec'+str(binning),speed_info_sec)
    
if __name__ == "__main__":
    N = 2 #number to be evaluated cells
    binning = 160 #binning to x nm
    px_extent = 4  #def of number of px surrounding fp outline
    resultpath = '/Volumes/Reindeer/TrackingVSGAtto/Trc/Eval_outgoing/Maps_Outline_200427'
